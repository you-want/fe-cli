...
const tdataWebpack = require('./tdata-webpack')
const antdWebpack = require('./antd-webpack')
const vue3Antd = require('./vue3-antd')
...
module.exports = function (params) {
    ...
    inquirer.prompt([{
        ...
        if (answers.clitype === '1）vue2+element模板') {
            tdataWebpack(params)
        }
        if (answers.clitype === '2）vue2+antd模版') {
            antdWebpack(params)
        }
        if (answers.clitype === '3）vue3+antd模版') {
            vue3Antd(params)
        }
        if (answers.clitype === '4）其他模板') {
