...
module.exports = function (params) {
    ...
    spinner.start();
    try {
        download('gitlab:git.100tal.com:jituan_bigdata_fe/the_rock_plan_template', params.name, {
            clone: true,
            type: params.type
        }, (error) => {
            spinner.stop();
            ...
            console.log(`cd ${params.name} && npm install \n`)
            ...
            inquirer.prompt([{
                ...
                if (answers.install != 'false' && answers.install != 'N' && answers.install != 'n') {
                    ...
                    spinner.start();
                    shell.exec("cd " + params.name + " && npm i", function (err, stdout, stderr) {
                        if (err) {
                            spinner.fail();
